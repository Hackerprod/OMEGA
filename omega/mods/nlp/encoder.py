...
class ContinuousTextEncoder(BaseEncoder):
    ...
    def _build_projection(self, vocab_size: int) -> np.ndarray:
        ...
        matrix = self.rng.standard_normal((self.d_model, vocab_size)) * scale
    ...
    def encode_text(self, text: str) -> np.ndarray:
        ...
        prev = np.zeros(self.d_model, dtype=np.float64)
        for char in text:
    ...
    def encode_file_to_memmap(
        ...
        mmap = np.memmap(output_path, mode="w+", dtype=dtype, shape=(total_chars, self.d_model))
        prev = np.zeros(self.d_model, dtype=np.float64)
        offset = 0
        remaining = max_chars
        with text_path.open("r", encoding=encoding) as fh:
